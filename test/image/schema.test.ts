...
test("schemas expose the reduced image controls and reject obsolete arguments", () => {
  ...
  for (const options of [{ ...IMAGE_DEFAULTS }, { model: "gpt-image-unknown" }, { n: 1 }, { quality: "invalid" }, { output_format: "png" }, { stream: true }, { partial_images: 0 }, { moderation: "off" }, { mask: { path: "mask.png" } }, { images: [{ file_id: "file-test" }] }, { output_compression: 60 }, { input_fidelity: "high" }, { user: "test" }]) assert.equal(Value.Check(ImageSchema, { prompt: "hello", ...options }), false);
