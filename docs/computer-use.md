...
`runtime.ts` discovers the absolute application bundle from `OPENAI_CODEX_COMPUTER_APP` or `/Applications/ChatGPT.app`, checks shipped executables and modules, then creates a private temporary working directory with a symlink to shipped node_modules. No project-local executables are discovered.

The child launches the shipped `unified-computer-use/scripts/launch.mjs` with computer-only surfaces. That launcher configures trusted Sky service RPC and the CUA JavaScript banner. The bridge supplies the shipped `node`, `node_repl`, module directories, and `CODEX_CLI_PATH` just as the desktop integration does. The sanitized PATH includes the bundle's Resources directory: Sky needs the packaged `codex` for native configuration resolution. Omitting it caused `codex app-server exited before returning a response` in testing.
...
At `agent_settled`, invoke bounded `turn_ended(Stop, session_id, turn_id)`, then unconditionally disconnect/stop the bridge-owned runtime and remove its temporary workspace, even if the hook succeeds or fails. Preserve only in-memory session app grants. This is a conservative workaround for intermittent native virtual-cursor overlays: the installed Sky JS service does not register a native turn-ended handler, so a successful node_repl hook response alone does not release the overlay. The user confirmed that resetting/disconnecting removes it. Do not directly synthesize undocumented native IPC or kill the shared Sky service. At reset/shutdown, attempt bounded `turn_ended(Interrupt, ...)`, stop the bridge-owned process group and delete its temporary workspace. Never kill other Codex or Sky processes. The model must reinitialize all JS variables and app bindings after a settled task or reset. Low-level `agent_end` is deliberately not a cleanup boundary: pi 0.85.1 may automatically retry or compact/continue after that event. Settled cleanup rechecks `ctx.isIdle()` inside the serialized queue to avoid interrupting new work started by another extension. Ordinary cleanup does not revoke app grants or invalidate queued work for the next task. The generation value identifies each process initialization, not the internal kernel state after errors.

Shutdown first ends stdin and gives the owned process group 300 ms to exit gracefully, then escalates to SIGTERM (700 ms) and SIGKILL (500 ms) if needed. Cancellation/transport failures skip the EOF grace period and signal the process group immediately. It checks the process group rather than just the launcher. `computer status` exposes the most recent cleanup reason, hook result, generation, elapsed time, exit code/signal, signals sent, observed group termination and workspace removal. These diagnostics contain no code or UI contents. A successful process shutdown does not prove that a native overlay disappeared; verify visually on a real desktop. No shared Sky service is killed.
...
Native Sky internals use a different length-prefixed JSON-RPC connection to their own signed service. This extension does not directly connect to that socket or emulate its peer authentication.
...
Observed after setting up the complete shipped runtime path and sandbox launcher:

- First Calculator call: ~1115 ms, including MCP startup and initial documentation.
- Screenshot on same generation: ~76 ms.
- AX read on same generation: ~54 ms.
- Original hook-only test: the hook succeeded and the connection remained alive. This did **not** verify cursor disappearance and is no longer the cleanup behavior; current cleanup always releases the connection.

These are one local sample of tool latency, not end-to-end model latency or a speed guarantee. Earlier disposable probes also verified Safari navigation and repository viewing; the new persistent implementation's smoke is deliberately read-only.
