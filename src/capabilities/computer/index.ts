...
export function registerComputer(pi: ExtensionAPI, root: string, session = new ComputerSession()): ControlManagement {
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    ...
    if (ctx.hasUI && cleanup?.shutdown && (!cleanup.shutdown.processGroupStopped || !cleanup.shutdown.workspaceRemoved)) {
      ctx.ui.notify("Computer cleanup was incomplete. Check /openai-codex-enhance computer status; native cursor release is not verified.", "warning");
    }
  });
