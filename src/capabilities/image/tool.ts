...
import { recentImages } from "./history.ts";
...
export function imageTool(deps: ImageDependencies): ToolDefinition<typeof ImageSchema, Details> {
  return {
    ...
    description: "Generate or edit images with OpenAI GPT Image over the Codex HTTP endpoint. No inputs means generation; images or num_last_images_to_include means editing. Supports local paths, image URLs/data URLs, size, background, quality and moderation. Quality defaults to high and moderation to auto. Moderation low does not disable safety policies and its Codex backend effect is unverified. Fixed internally: gpt-image-2, one image, PNG, SSE with no requested partial images. These defaults are not tool arguments. Requested dimensions may not be honored; observable mismatches are reported. Input files must be PNG/JPEG/WebP under 50 MB each, at most 16 images and 100 MiB combined. Inspect local reference images with read before editing. Specify exactly one image source per item and never combine images with num_last_images_to_include. Preserve unchanged details explicitly in the prompt. Generated originals are saved locally, with at most 4 small previews returned. Reuse original saved paths for later edits. Requests can take several minutes, consume image quota, and are never automatically retried.",
    ...
    async execute(callId, args, signal, onUpdate, ctx) {
      ...
      if (!Value.Check(ImageSchema, args)) throw new Error("Invalid codex_image arguments; model/count/format/streaming are fixed internally. Use the current tool schema.");
      if (args.images && args.num_last_images_to_include) throw new Error("images and num_last_images_to_include are mutually exclusive.");
      const sources = args.images ?? (args.num_last_images_to_include ? recentImages(contextMessages(ctx.sessionManager.buildContextEntries()), args.num_last_images_to_include) : []);
